...
function writeArchiveIndex(archiveDir: string, issues: ArchivedIssue[]): void {
  const lines = [
    ...
    ...issues
      ...
      .map(
        (i) =>
          `| ${i.id} | ${i.title} | ${i.tags.join(", ")} | ${i.completed} | ${i.quarter} |`
      ),
    "",
...
export function generateBoardMarkdown(
  ...
  for (const folder of STATUS_FOLDERS) {
    ...
    if (folder === "5-done") {
      ...
      const showAssignee =
        ...
        folderIssues.some(
          (i) =>
            typeof i.frontmatter.assignee === "string" && i.frontmatter.assignee
        );
      const showReview =
        folder === "4-review" &&
        folderIssues.some(
          (i) =>
            typeof i.frontmatter.review === "string" && i.frontmatter.review
        );
      ...
      for (const issue of folderIssues) {
        ...
        if (hasDeps) {
          ...
          row.push(
            Array.isArray(deps) && deps.length > 0 ? deps.join(", ") : "—"
          );
        }
        lines.push(`| ${row.join(" | ")} |`);
      }
  ...
  if (archivedCount > 0) {
    lines.push(
      `${archivedCount} issues archived → [archive/INDEX.md](archive/INDEX.md)`
    );
  } else {
 ...
 */
export function boardToJson(data: BoardData): Record<string, unknown> {
  return {
