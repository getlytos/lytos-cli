...
export const moveCommand = new Command("move")
  .description(
    "Move an issue to another stage — update status, move the file, regenerate the board"
  )
  .argument("<issue-id>", "Issue ID (e.g. ISS-0029)")
  .argument("<stage>", `Target stage (${MOVABLE_STAGES.join(", ")})`)
  .option(
    "--force",
    "Move even when origin is ahead or the issue is claimed on origin",
    false
  )
  ...
  .on("--help", () => {
    ...
    console.log(
      "  lyt move ISS-0053 1-backlog    # send it back to the backlog"
    );
    console.log("  lyt move ISS-0053 4-review --json");
    console.log("");
    console.log("Stages with a dedicated verb are refused on purpose:");
    console.log(
      "  3-in-progress → lyt start   (branch, assignee, origin check)"
    );
    console.log(
      "  5-done        → lyt close   (checklist gate, commits capture)"
    );
  })
  .action(
    (
      ...
      if (issue.dir === stage) {
        if (opts.json) {
          console.log(
            JSON.stringify({ status: "already-there", id: issueId, stage })
          );
          return;
        }
      ...
      if (opts.json) {
        console.log(
          JSON.stringify(
            { status: "moved", id: issueId, from, to: stage },
            null,
            2
          )
        );
