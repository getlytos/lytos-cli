...
export const budgetCommand = new Command("budget")
  .description("Aggregate pipeline cost vs a ceiling — non-zero exit when breached")
  .option("--max-usd <amount>", "Ceiling on total cost_usd")
  .option("--max-issues <count>", "Ceiling on issue count")
  .option("--all", "Scope the whole board instead of the in-flight pipeline", false)
  .option("--json", "Output as JSON", false)
  .on("--help", () => {
    console.log("");
    console.log("Ceiling resolution: --max-* flags override budget_usd / budget_issues in sprint.md.");
    console.log("Scope (default): 2-sprint, 3-in-progress, 4-review, parked. --all widens it.");
    console.log("");
    console.log("Examples:");
    console.log("  lyt budget --max-usd 50");
    console.log("  lyt budget --max-issues 20 --json");
  })
  .action((opts: { maxUsd?: string; maxIssues?: string; all?: boolean; json?: boolean }) => {
    ...
    const report = computeBudget(lytosDir, { maxUsd, maxIssues, all: opts.all });
    ...
    if (opts.json) {
      ...
      if (!report.hasCeiling) {
        info(`Spend: ${totals}`);
        warn("No ceiling set — pass --max-usd / --max-issues or add budget_usd / budget_issues to sprint.md.");
      } else if (report.overBudget) {
        error(`Over budget: ${totals}`);
        for (const b of report.breaches) console.error(`  ${yellow("!")} ${b}`);
      } else {
        ok(`Within budget: ${totals}`);
        if (report.maxUsd !== null) console.error(`  ${green("✓")} ${dim(`cost ≤ $${report.maxUsd.toFixed(2)}`)}`);
        if (report.maxIssues !== null) console.error(`  ${green("✓")} ${dim(`issues ≤ ${report.maxIssues}`)}`);
      }
      console.error("");
    }
