...
import { scanOriginNotes, cherryPickNotes, type NoteCommit } from "../lib/pull-notes.js";
...
function ensureGitContext(cwd: string, mainBranch: string): string | null {
  ...
  try {
    ...
    warn(`Could not fetch origin/${mainBranch} — using the last known origin state.`);
  }
  try {
    execFileSync("git", ["rev-parse", "--verify", `origin/${mainBranch}`], { cwd, stdio: "pipe" });
  } catch {
...
function printMixedRefusals(mixed: NoteCommit[]): void {
  console.error("");
  warn(`${mixed.length} commit${mixed.length > 1 ? "s" : ""} refused — touching code as well as .lytos/ (repatriation is for notes; these need a merge):`);
...
export const pullNotesCommand = new Command("pull-notes")
  .description("Cherry-pick .lytos-only commits from origin/main onto the current branch, then regenerate the board")
  .option("--dry-run", "List what would be cherry-picked, change nothing", false)
  .option("--main <branch>", "Name of the main branch on origin", "main")
  .on("--help", () => {
    console.log("");
    console.log("Examples:");
    console.log("  lyt pull-notes             # cherry-pick every pending note commit");
    console.log("  lyt pull-notes --dry-run   # list them without touching the tree");
    console.log("");
    console.log("A note commit touches ONLY .lytos/. Commits mixing .lytos/ and");
  ...
  .action((opts: { dryRun?: boolean; main?: string }) => {
    ...
    try {
      ...
      error(`Could not scan origin/${mainBranch}: ${err instanceof Error ? err.message : String(err)}`);
      process.exit(1);
    }

    if (scan.notes.length === 0 && scan.mixed.length === 0) {
      info(`No .lytos-only commits on origin/${mainBranch} missing from HEAD. Nothing to repatriate.`);
      return;
    }

    if (scan.notes.length > 0) {
      console.error("");
      info(`${scan.notes.length} note commit${scan.notes.length > 1 ? "s" : ""} on origin/${mainBranch} missing from HEAD:`);
    ...
    ok(`${result.picked.length} note commit${result.picked.length > 1 ? "s" : ""} cherry-picked (-x) onto the current branch`);
