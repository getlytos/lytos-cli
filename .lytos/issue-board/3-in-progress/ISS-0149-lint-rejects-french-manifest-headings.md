...
- [ ] Manifest section headings and the language-dependent placeholder labels are defined once,
      per language, and imported by both `templates.ts` and `linter.ts` — verify: auto
- [ ] A manifest generated from the French template passes `lyt lint`'s section and placeholder
      checks — verify: auto
- [ ] A manifest generated from the English template still passes `lyt lint` — verify: auto
- [ ] A manifest missing a section still fails lint, in both English and French — verify: auto
- [ ] A regression test fails if a template heading and the linter's expectation ever diverge
