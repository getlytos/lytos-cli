# Issue Board

> This file is generated by `lytos board`. The source of truth is the YAML frontmatter of each issue.
>
> **Last generated**: 2026-05-25
> **Next number**: ISS-0081

---

## Issue index

### 0-icebox (ideas)

_No issues._

### 1-backlog (prioritized)

| # | Title | Priority | Effort | Depends |
|---|---|---|---|---|
| [ISS-0040](1-backlog/ISS-0040-cloud-app-compatibility.md) | Verify Lytos compatibility with Claude Code Desktop and Codex web app | P2-normal | S | — |
| [ISS-0042](1-backlog/ISS-0042-claim-as-draft-pr.md) | lyt claim: atomic claim via branch + push + draft PR (full race-proof design) | P1-high | L | ISS-0041 |
| [ISS-0043](1-backlog/ISS-0043-board-all-remote.md) | lyt board --all --remote: lead-view from origin, not local | P2-normal | S | — |
| [ISS-0047](1-backlog/ISS-0047-skill-import.md) | lyt skill: import agentskills.io skills into a Lytos project | P2-normal | L | ISS-0046 |
| [ISS-0056](1-backlog/ISS-0056-reduce-board-regeneration-friction.md) | Reduce friction from mandatory `lyt board` regeneration on every issue move | P2-normal | S | — |
| [ISS-0057](1-backlog/ISS-0057-validate-lytos-on-real-feature.md) | Validate Lytos on a real feature workflow with human controls | P1-high | M | — |
| [ISS-0060](1-backlog/ISS-0060-doctor-false-positives.md) | lyt doctor: fix two false-positive classes (repo-relative links, archived deps) | P2-normal | S | — |
| [ISS-0067](1-backlog/ISS-0067-propagate-auditor-section-to-method.md) | Propagate implementer/auditor section to lytos-method | P2-normal | XS | — |
| [ISS-0068](1-backlog/ISS-0068-extract-review-prompt-template.md) | Extract review prompt template out of src/lib/review.ts | P3-low | S | — |
| [ISS-0069](1-backlog/ISS-0069-countchecklist-skip-code-fences.md) | countChecklist: skip checkboxes inside fenced code blocks | P2-normal | S | — |
| [ISS-0075](1-backlog/ISS-0075-adr-scaffold-and-index.md) | Scaffolder `adr/` dans `lyt init` avec INDEX.md et template | P2-normal | S | — |
| [ISS-0076](1-backlog/ISS-0076-ai-wrapper-integration.md) | AI wrapper integration — write ai_implementer / tokens / cost (phase 4 of schema v2) | P2-normal | M | ISS-0074 |
| [ISS-0077](1-backlog/ISS-0077-migrate-frontmatter.md) | lyt migrate-frontmatter — backfill schema_version + lifecycle fields (phase 5 of schema v2) | P3-low | M | ISS-0074 |
| [ISS-0079](1-backlog/ISS-0079-gitignore-board-md.md) | Gitignore BOARD.md sur lytos-cli (mirror lytos-app ADR-0010) | P2-normal | XS | — |
| [ISS-0080](1-backlog/ISS-0080-method-scaffold-gitignore-board-md.md) | Inclure BOARD.md dans le .gitignore par défaut du scaffold lyt init | P2-normal | XS | ISS-0079 |

### 2-sprint (committed)

_No issues._

### 3-in-progress (in dev)

_No issues._

### 4-review (review/test)

_No issues._

### 5-done (recently completed)

| # | Title | Completed |
|---|-------|-----------|
| [ISS-0059](5-done/ISS-0059-lyt-review-command.md) | Add lyt review command for cross-model code audit | 2026-04-22 |
| [ISS-0064](5-done/ISS-0064-archive-cleanup.md) | Archive completed housekeeping issues and restore a clean main worktree | 2026-04-22 |
| [ISS-0065](5-done/ISS-0065-release-1-0-0.md) | Release 1.0.0 | 2026-04-22 |
| [ISS-0066](5-done/ISS-0066-release-1-0-1.md) | Release 1.0.1 after premature 1.0.0 publish | 2026-04-22 |
| [ISS-0074](5-done/ISS-0074-frontmatter-schema-v2.md) | Frontmatter schema v2 (auditabilité durable) | 2026-05-25 |
| [ISS-0078](5-done/ISS-0078-release-1-1-0.md) | Release 1.1.0 — frontmatter schema v2 | 2026-05-25 |

### Archive

54 issues archived → [archive/INDEX.md](archive/INDEX.md)

---

*Generated by `lytos board` — the YAML frontmatter is the source of truth.*
